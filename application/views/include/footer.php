...
<div class="footer py-5">
    <div class="container">
		 ...
		 <?php
             ...
             $i=0;
             foreach ($coinListtData->data as $res)
				{
				    if($i==7) continue; 
				    $ex_code=$res->id;
    ...
    <span class="badge badge-light">  <img src="<?php echo $img_src;?>" width="15px"/> <?php echo $res->name;?> <span class="badge badge-warning"><?php echo $res->symbol;?></span></span>
</a>
	<?php
		++$i;
		}
	...
	<a href="<?php echo base_url() ?>coin/<?php echo $res->id; ?>">
    <span class="badge badge-light"> <?php echo $res->symbol;?> <?php echo number_format($res->marketCapUsd/$totalCap*100, 1);?>%</span>
</a>
	<?php
		++$i;
		}
					...
					<?php $i=0; foreach ($coinChange24DesSort as $key => $value)
                        {
							if($i>4)   continue; ?>
                        <ul>
							<li><span class="coin-code"><a href="<?php echo base_url() ?>coin/<?php echo $coinId[$key]; ?>"><b><?php echo $coinName[$key];?></b> <small><?php echo $coinCode[$key];?></small></a></span><?php echo strtok($convertSymbol, " ");?><?php echo custom_price_format(str_replace(',','',$coinPrice[$key]/$convertRate)); ?> <span class="change-g"><?php echo round($coinChange24[$key],2);?>%</span></li>
						</ul>
                    <?php ++$i;} ?>
			</div>
			<div class="col-sm">
				<h5>Top 5 Rank Coins</h5>
					<?php for($i=0;$i<5;$i++) { ?>
                      <ul>
						<li><span class="coin-code"><a href="<?php echo base_url() ?>coin/<?php echo $coinId[$i]; ?>"><b><?php echo $coinName[$i];?></b> <small><?php echo $coinCode[$i];?></small></a></span><?php echo strtok($convertSymbol, " ");?><?php echo custom_price_format(str_replace(',','',$coinPrice[$i]/$convertRate)); ?>
                          ...
                          foreach ($coinChange24Sort as $key => $value)
								{
									...
									if($i>4)   continue; ?>
                            <ul>
							<li><span class="coin-code"><a href="<?php echo base_url() ?>coin/<?php echo $coinId[$key]; ?>"><b><?php echo $coinName[$key];?></b> <small><?php echo $coinCode[$key];?></small> </a></span><?php echo strtok($convertSymbol, " ");?><?php echo custom_price_format(str_replace(',','',$coinPrice[$key]/$convertRate)); ?> <span class="change-l"><?php echo round($coinChange24[$key],2);?>%</span></li>
						</ul>
                    <?php ++$i;}} ?>
			</div>
		</div>
                        ...
                        <div class="col-md-12">
                            <div class="d-flex">
                                <div class="f-nav mr-auto">
                                    ...
                                    <a href="<?php echo $settingData[0]['google_link'] ?>" target="_blank"><i class="fa fa-linkedin"></i> <b>Linkedin</b></a></small></p>
                                </div>
                                 <p><?php echo $settingData[0]['copyright'] ?><br><small>Data provided by <a href="https://coincap.io/" rel="nofollow" target="_blank"> CoinCap</a></small></p>
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="ticker-bottom">
	...
	for($j=0;$j<25;$j++)
				{ 	
					$img_src=base_url().'assets/images/coins/'.$coinId[$j].'.png';
			        $file_path=FCPATH.'assets/images/coins/'.$coinId[$j].'.png';
			        if (!file_exists($file_path)) $img_src=base_url().'assets/images/default.png';
	?>
<div class="ticker__item"><img src="<?php echo $img_src ?>" /> <span><a href="<?php echo base_url() ?>coin/<?php echo $coinId[$j]; ?>"><?php echo $coinCode[$j];?></a></span> <div class="cmlp-price"><?php echo strtok($convertSymbol, " ");?><?php echo custom_price_format(str_replace(',','',$coinPrice[$j]/$convertRate)); ?></div> <span><i class="fa fa-caret-<?php echo $coinChange24[$j] > 0 ? 'up':'down'?>"></i> <?php echo str_replace('-','',round($coinChange24[$j],2));?>%</span></div>
	<?php } ?>
	</div>
</div>
<?php } ?>
